...
import com.ir.tools.avro.config.PrintConverters;
...
public class GenericData {
    ...
    public String toString(Object datum) {
        StringBuilder buffer = new StringBuilder();
        toString(datum, buffer, new IdentityHashMap<Object, Object>(128), null);
        return buffer.toString();
    }
    ...
    protected void toString(Object datum, StringBuilder buffer, IdentityHashMap<Object, Object> seenObjects, String fieldName) {
        ...
        if (isRecord(datum)) {
            ...
            for (Field f : schema.getFields()) {
                toString(f.name(), buffer, seenObjects, null);
                buffer.append(": ");
                toString(getField(datum, f.name(), f.pos()), buffer, seenObjects, f.name());
            ...
            for (Object element : array) {
                toString(element, buffer, seenObjects, fieldName);
                if (i++ < last)
                    buffer.append(", ");
            }
            buffer.append("]");
            ...
            Map<Object, Object> map = (Map<Object, Object>) datum;
            for (Map.Entry<Object, Object> entry : map.entrySet()) {
                toString(entry.getKey(), buffer, seenObjects, null);
                buffer.append(": ");
                toString(entry.getValue(), buffer, seenObjects, null);
                if (++count < map.size())
                    buffer.append(", ");
            }
            buffer.append("}");
